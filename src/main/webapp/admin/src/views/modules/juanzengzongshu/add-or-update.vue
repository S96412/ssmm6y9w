      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="总数" prop="zongshu">
          <el-input v-model="ruleForm.zongshu" 
              placeholder="总数" clearable  :readonly="ro.zongshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="总数" prop="zongshu">
              <el-input v-model="ruleForm.zongshu" 
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `juanzengzongshu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.juanzengzongshuCrossAddOrUpdateFlag = false;
						 this.parent.search();
