...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          shouzengzhanghao: [
                { required: true, message: '受赠账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          shouzengxingming: [
                { required: true, message: '受赠姓名不能为空', trigger: 'blur' },
          ],
          nianling: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shouzengfang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shouzengfangCrossAddOrUpdateFlag = false;
						 this.parent.search();
