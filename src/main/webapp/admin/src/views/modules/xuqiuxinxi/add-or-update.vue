<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zhengming" label="证明" prop="zhengming">
          <file-upload
          tip="点击上传证明"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhengming?ruleForm.zhengming:''"
          @change="zhengmingUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.zhengming" label="证明" prop="zhengming">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhengming.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
            ...
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="需求用途" prop="xuqiuyongtu">
                <el-input
                  ...
                  :rows="8"
                  placeholder="需求用途"
                  v-model="ruleForm.xuqiuyongtu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xuqiuyongtu" label="需求用途" prop="xuqiuyongtu">
                    <span>{{ruleForm.xuqiuyongtu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhengming = true;
          ...
          if(o=='xuqiuyongtu'){
            this.ruleForm.xuqiuyongtu = obj[o];
	    this.ro.xuqiuyongtu = true;
            continue;
          }
          if(o=='dizhi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.shouzengzhanghao!=''&&json.shouzengzhanghao){
                this.ruleForm.shouzengzhanghao = json.shouzengzhanghao
	    		this.ro.shouzengzhanghao = true;
		}
		if(json.shouzengxingming!=''&&json.shouzengxingming){
                this.ruleForm.shouzengxingming = json.shouzengxingming
	    		this.ro.shouzengxingming = true;
		}
	...
	this.ruleForm.xiangxishuoming = this.ruleForm.xiangxishuoming.replace(reg,'../../../ssmm6y9w/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.zhengming!=null) {
		this.ruleForm.zhengming = this.ruleForm.zhengming.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    zhengmingUploadChange(fileUrls) {
	this.ruleForm.zhengming = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
